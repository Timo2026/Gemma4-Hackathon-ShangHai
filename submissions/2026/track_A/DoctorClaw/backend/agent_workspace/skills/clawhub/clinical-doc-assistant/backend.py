...
async def generate_document(
    ...
    # Call Gemma 4 — no PHI is stored after this call
...
async def _call_llm(prompt: str) -> str:
    ...
    api_key = os.environ.get("LLM_API_KEY") or os.environ.get("AGENT_API_KEY")
    if not api_key:
        raise HTTPException(500, "Gemma API key not configured on server.")
    ...
    if response.status_code != 200:
        raise HTTPException(502, f"Gemma API error: {response.text}")
