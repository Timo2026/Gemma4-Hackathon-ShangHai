...
**PubMed-Search** is a DoctorClaw Agent Skill designed to help researchers search and analyze biomedical literature from PubMed. It provides powerful search capabilities, metadata extraction, deep paper analysis, and full-text PDF downloads for open access articles.
...
```bash
# Clone to DoctorClaw Agent skills directory
git clone https://github.com/JackKuo666/pubmed-search-skill.git ~/.doctorclaw/skills/pubmed-search-skill
```

### Method 3: Manual Installation

1. Download the project ZIP or clone to local
2. Copy the `pubmed-search-skill` folder to DoctorClaw Agent skills directory:
   - **macOS/Linux**: `~/.doctorclaw/skills/`
   - **Windows**: `%USERPROFILE%\.doctorclaw\skills\`
3. Ensure the folder structure is:

```
~/.doctorclaw/skills/pubmed-search-skill/
...
cd ~/.doctorclaw/skills/pubmed-search-skill
uv venv
...
Restart DoctorClaw Agent or reload skills, then enter in conversation:
...
```
pubmed-search-skill/
├── SKILL.md              # DoctorClaw Agent skill definition
├── pubmed_search.py      # Core search script
├── README.md             # Documentation
├── requirements.txt      # Python dependencies
└── .env.example          # Environment variable examples
