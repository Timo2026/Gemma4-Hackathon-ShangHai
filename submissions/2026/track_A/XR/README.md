...
### 项目定位
XR Lite Version for GDG Gemma 4 Hackathon 2026 是为赛道 A 准备的公开评审项目。它展示 Gemma 4 如何在本地或私有 OpenAI-compatible endpoint 中，把自然语言生物医学分析需求转化为可审计的公开工作流：模型先通过 Native guided tool calling 选择工作流，API 再基于公开 synthetic sample data 和公开 Docker 工具链执行分析，并由 Gemma 4 对结果进行简要解释。
...
截图建议见 `logs_agent_trace.png`, `logs_tool_calls.png`, `logs_agent_memory.png`。运行日志可直接打开上述 `.log` 文件或用 `tail -f` 截图；Agent artifacts 截图位于 `.review-data/jobs/<jobId>/agent-memory.json`、`tool-calls.json` 和 `agent-trace.json`。
...
See `logs_agent_trace.png`, `logs_tool_calls.png`, `logs_agent_memory.png` for recommended screenshots. Runtime logs can be opened directly from the `.log` files above or captured with `tail -f`; Agent artifact screenshots are under `.review-data/jobs/<jobId>/agent-memory.json`, `tool-calls.json`, and `agent-trace.json`.
