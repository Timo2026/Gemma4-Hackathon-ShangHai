...
基于 [Google AI Edge Gallery](https://github.com/google-ai-edge/gallery) 开源项目，利用 **MediaPipe LLM Inference API** 在设备本地运行 **Gemma 4** 系列 AI 模型，无需联网即可完成智能对话、诈骗检测、跨应用自动化操作等任务。
...
| 需求 | Gemma 4 如何满足 |
| --- | --- |
| **完全离线运行** | Gemma 4 提供从 2B 到 31B 的多种规格，2B/4B 版本可在主流 Android 手机本地运行，无需联网 |
...
| **量化部署** | int4 量化后 Gemma-4-E2B 仅 ~2.4GB，8GB 内存的设备即可运行 |
...
┌─────────────────────────────────────────────────────────────────┐
│                    执行层 (Action Layer)                          │
│         📱 AccessibilityService · 📞 电话 · 💬 短信 · 🔊 TTS     │
└─────────────────────────────────────────────────────────────────┘
```
...
内置 **16 个**技能（挂号、缴费、地图、心情追踪等），支持从社区精选列表、URL 或本地加载更多。
...
| | Gemma 4 / Gemma 3n / Qwen2.5 | 设备端 AI 模型（int4/int8 量化） |
...
└── src/
    ...
    ├── model_allowlist.json       # 可用 AI 模型列表
    │
...
- **JDK 21**
- **Android SDK 35**
- **Android 12+ (API 31+)** 设备（建议 8GB+ 内存）
...
| 模型 | 大小 | 最低内存 | 量化 | 支持能力 |
| --- | --- | --- | --- | --- |
| **Gemma-4-E2B-it** | ~2.4 GB | ≥ 8 GB | int4 | 对话 · 图像理解 · 音频理解 · 思维链 · 32K 上下文 |
| **Gemma-4-E4B-it** | ~3.4 GB | ≥ 12 GB | int4 | 对话 · 图像理解 · 音频理解 · 思维链 · 32K 上下文 |
| **Gemma-3n-E2B-it** | ~3.4 GB | ≥ 8 GB | int4 | 对话 · 图像理解 · 音频理解 · 4K 上下文 |
| **Gemma-3n-E4B-it** | ~4.6 GB | ≥ 12 GB | int4 | 对话 · 图像理解 · 音频理解 · 4K 上下文 |
| **Gemma3-1B-IT** | ~557 MB | ≥ 6 GB | int4 | 对话 · 提示实验 |
| **Qwen2.5-1.5B-Instruct** | ~1.5 GB | ≥ 6 GB | q8 | 对话 · 提示实验 |
| **DeepSeek-R1-Distill-Qwen-1.5B** | ~1.75 GB | ≥ 6 GB | q8 | 对话 · 提示实验 · 推理链 |

> 推荐配置：日常使用 Gemma-4-E2B-it（2.4GB，8GB 设备可运行），高端设备可选 E4B 获得更强推理能力。
